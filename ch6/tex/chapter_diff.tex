...
%DIFDELCMD <   %%%
\DIFdel{We report in situ measurements through the auroral acceleration region that
  reveal the prevalence of extremely nonthermal monoenergetic electron
  distributions. These auroral primaries are indicative of source populations in
  the plasma sheet well described as kappa distributions with
  $\kappa \lesssim$~2. We show from observations and modeling how this large
  deviation from Maxwellian form modifies the acceleration potential required to
  drive current closure through the auroral ionosphere. This violation of the
...
%DIFDELCMD < %%%
\DIFdelend %DIF >  \begin{abstract}
%DIF >    We report in situ measurements through the auroral acceleration region that
%DIF >    reveal the prevalence of extremely nonthermal monoenergetic electron
%DIF >    distributions. These auroral primaries are indicative of source populations in
%DIF >    the plasma sheet well described as kappa distributions with
%DIF >    $\kappa \lesssim$~2. We show from observations and modeling how this large
%DIF >    deviation from Maxwellian form modifies the acceleration potential required to
%DIF >    drive current closure through the auroral ionosphere. This violation of the
%DIF >    classically assumed relationship between field-aligned current and auroral
%DIF >    acceleration potential has fundamental implications for present-day
%DIF >    understanding of magnetosphere-ionosphere coupling that should be incorporated
%DIF >    in global magnetospheric models.
%DIF >  \end{abstract}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \begin{equation} \label{ch6:eqKnight} j_{\parallel,i} ( \, \Delta \Phi ; T_m,
    n_m, R_B ) = - e n_m \Big ( \DIFdelbegin %DIFDELCMD < \dfrac{k_B T_m}{2 \pi m_e} %%%
\DIFdelend \DIFaddbegin \dfrac{T_m}{2 \pi m_e} \DIFaddend \Big )^{\frac{1}{2}}
    R_B \Bigg [ 1 - \Big ( 1 - R_B^{-1} \Big ) \textrm{exp} \Big \{ - \DIFdelbegin %DIFDELCMD < \dfrac{e
%DIFDELCMD <       \Delta \Phi}{k_B T_m ( R_B - 1 )} %%%
\DIFdelend \DIFaddbegin \dfrac{\bar{\phi}}{( R_B - 1 )} \DIFaddend \Big \} \Bigg],
  ...
  precipitating electrons at the magnetospheric source\DIFaddbegin \DIFadd{,
  $\bar{\phi} \equiv \frac{\Delta \Phi}{T_m}$, }\DIFaddend and $m_e$ is the electron mass.
  ...
  \begin{equation} \label{ch6:eqMax1D} f_{M}( E ; T, n) \, = n \Big (
    \DIFdelbegin %DIFDELCMD < \dfrac{m}{2 \pi k_{B} T} %%%
\DIFdelend \DIFaddbegin \dfrac{m}{2 \pi T} \DIFaddend \Big )^{\frac{3}{2}} \textrm{exp} \Big (
    - \DIFdelbegin %DIFDELCMD < \dfrac{E}{ k_B T } %%%
\DIFdelend \DIFaddbegin \dfrac{E}{ T } \DIFaddend \Big ).
  ...
  % \begin{linenomath*}
    \begin{equation} \label{ch6:eqKappa1D} f_{\kappa}(E; T, n, \kappa) = n \Bigg
      ( \DIFdelbegin %DIFDELCMD < \dfrac{m}{2 \pi (\kappa - \frac{3}{2}) k_{B} T \, } %%%
\DIFdelend \DIFaddbegin \dfrac{m}{2 \pi (\kappa - \frac{3}{2}) T \, } \DIFaddend \Bigg
      )^{\frac{3}{2}} \dfrac{\Gamma \big ( \kappa + 1 \big ) }{\Gamma \big (
        \kappa - \frac{1}{2} \big ) } \Bigg ( 1 + \DIFdelbegin %DIFDELCMD < \dfrac{E}{ \big ( \kappa -
%DIFDELCMD <         \frac{3}{2} \big ) k_B T } %%%
...
%DIFDELCMD <         j_{\parallel,i} ( \, \Delta \Phi ; T_m, n_m, \kappa, R_B ) &= - e n_m \sqrt{ \dfrac{ k_B T_m (\kappa -
%DIFDELCMD <             \frac{3}{2})}{ 2 \pi m_e}} \dfrac{\Gamma (\kappa + 1)}{\Gamma (\kappa - \frac{1}{2})} \dfrac{R_B}{\kappa (\kappa - 1)} \\
%DIFDELCMD <         &\times \Bigg[1 - \Big(1 - R_B^{-1} \Big) \Bigg(1+\dfrac{e \Delta \Phi}{ k_B
%DIFDELCMD <           T_m (\kappa - \frac{3}{2})(R_B - 1)} \Bigg)^{1-\kappa} \Bigg].
%DIFDELCMD <       \end{array}
%DIFDELCMD <     %%%
\DIFdelend \DIFaddbegin \begin{array}{ll}
  ...
  states that are far from thermal equilibrium \DIFaddbegin \DIFadd{(or ``far-equilibrium'' states),
  $\kappa \in ( \, \kappa_{\textrm{min}}, \kappa_t )$}\DIFaddend , \DIFdelbegin \DIFdel{$\kappa \in [ \, 3/2, \kappa_t
  )$, }\DIFdelend and ``near-equilibrium''
  states for which $\kappa \in [ \, \kappa_t, \infty )$, as well as evidence of
  a preferred ``fundamental state'' \DIFdelbegin \DIFdel{with
  $\kappa \simeq 1.63$ that corresponds to
  the minimum-entropy kappa distribution }\DIFdelend \DIFaddbegin \DIFadd{for
  ...
  \DIFadd{Under a Lagrangian interpretation of temperature, for all other parameters
  ...
  entropy monotonically increases until $\kappa$~=~$\kappa_{\textrm{Fund}}$}\DIFaddend . 

  \DIFaddbegin \DIFadd{For some applications \mbox{%DIFAUXCMD
\citep[e.g.,][]{Sutherland2012} }%DIFAUXCMD
it may be useful to
  ...
  energies. \mbox{%DIFAUXCMD
\citet{Livadiotis2010} }%DIFAUXCMD
show that this shifting is a consequence of
  stochastic particle deceleration.
}

  \DIFadd{We do not consider the Lagrangian temperature in this study. We moreover make
  ...
  distributions \mbox{%DIFAUXCMD
\citep{Livadiotis2010}}%DIFAUXCMD
.
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  kappa distribution.
\DIFdelbegin \DIFdel{For some applications \mbox{%DIFAUXCMD
\citep[e.g.,][]{Sutherland2012} }%DIFAUXCMD
it
...
\DIFaddbegin 
  ...
  \DIFadd{Both 1-D and 2-D fits are performed using Levenberg-Marquardt weighted
  least-squares minimization via the publicly available Interactive Data
  Language MPFIT library
  (http://cow.physics.wisc.edu/~craigm/idl/fitting.html). }\DIFaddend Throughout this study
  ...
  $j_{\parallel,i}$ that have been mapped to 100~km. These \DIFaddbegin \DIFadd{fits }\DIFaddend provide an
  additional means of constraining $\kappa$, or the degree to which the
  magnetospheric source population is in a nonthermal stationary state\DIFaddbegin \DIFadd{, and are
  likewise performed via the MPFIT library}\DIFaddend . The necessary elements are a
  ...
  the loss cone, and over all energies exceeding \DIFdelbegin \DIFdel{a lower energy bound determined
  by visual inspection (}\DIFdelend 200~eV \DIFdelbegin \DIFdel{for observations corresponding to orbit 1773 and
  }\DIFdelend \DIFaddbegin \DIFadd{(}\DIFaddend 500~eV\DIFdelbegin \DIFdel{for those }\DIFdelend \DIFaddbegin \DIFadd{) }\DIFaddend corresponding
  to orbit\DIFaddbegin \DIFadd{~1773 (orbit~}\DIFaddend 1843). \DIFaddbegin \DIFadd{These lower bounds on energy represent our best
  ...
  orbit. }\DIFaddend $j_{\parallel,i}$ is then obtained by mapping $j_\parallel$ to 100~km
  ...
  % \end{equation}
  % \end{linenomath*}
  where \DIFdelbegin \DIFdel{$\bar{\phi} = \Delta \Phi / k_B T $ and }\DIFdelend $\alpha_F = 1 / R_{B,F}$. $Q (\bar{\phi}, \alpha_F)$ is a sigmoid
  ...
  force.  \DIFaddbegin \DIFadd{($Q ( \bar{\phi}, \alpha_F )$ does not assume any particular field
  geometry or model, though in this chapter we use IGRF~11 as previously
  mentioned.) }\DIFaddend Since $n_F$ is fixed by observation, (\ref{ch6:eqBarbosa})
  ...
  \section{FAST Orbit 1773} \label{ch6:s1773}
  ...
  \DIFdelbegin \DIFdel{Performing }\DIFdelend \DIFaddbegin \DIFadd{Figure~\ref{ch6:Fig1}c shows the best-fit kappa parameters resulting from }\DIFaddend 2-D
  fits of the electron distributions within the precipitating loss cone\DIFdelbegin \DIFdel{shows }\DIFdelend \DIFaddbegin \DIFadd{. It is
  evident in this panel }\DIFaddend that many of the distributions are highly nonthermal,
  with $\kappa \lesssim$~2.0\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend Figure~\ref{ch6:Fig1}\DIFdelbegin \DIFdel{c) . Values }\DIFdelend \DIFaddbegin \DIFadd{d shows, for 2-D fits using
  either a kappa distribution (blue x's) or a Maxwellian distribution (red
  crosses), values }\DIFaddend of the reduced chi-squared statistic
  \DIFdelbegin \DIFdel{$\chi^2_{\mathrm{red}} = \frac{\sum_i Y(x) - y_i(x)}{\mathrm{dof}}$ for 2-D
  fits with a kappa distributionare at times }\DIFdelend \DIFaddbegin \begin{equation} \DIFadd{\label{ch6:eqchi2} \chi_{\mathrm{red}}^2 = \frac{\sum\limits_i^N \Big
      ( Y_i(x) - y_i(x) \Big )^2 / w_i^2}{F},
  }\end{equation}
  \DIFadd{where $i$ indexes each pitch-angle and energy bin used in the fitting
  ...
  \DIFadd{At times $\chi^2_{\textrm{red}}$ for the best-fit kappa distributions is }\DIFaddend much
  smaller than \DIFdelbegin \DIFdel{corresponding
  $\chi^2_{\textrm{red}}$ values for fits with a Maxwellian distribution(Figure~\ref{ch6:Fig1}c)}\DIFdelend \DIFaddbegin \DIFadd{that for the corresponding best-fit Maxwellian distribution}\DIFaddend , most
  ...
  \DIFaddbegin \DIFadd{Typical ranges of electron densities and temperatures in the distant plasma
  sheet are respectively 0.01--0.5~cm$^{-3}$ and 400-900~eV
  \mbox{%DIFAUXCMD
\citep{Kletzing2003,Paschmann2003}}%DIFAUXCMD
. Figures~\ref{ch6:Fig1}e--f show that
  within the inverted V precipitation, the observed (black diamonds) and
  best-fit kappa distribution (blue }\emph{\DIFadd{x}} \DIFadd{symbols) densities and temperatures
  ...
  \DIFdelbegin \DIFdel{Typical ranges of electron densities and temperatures in the distant plasma
  sheet are respectively 0.01--0.5~cm$^{-3}$ and 400-900~eV
  \mbox{%DIFAUXCMD
\citep{Kletzing2003,Paschmann2003}}%DIFAUXCMD
. Figures~\ref{ch6:Fig1}e--f show that
  within the inverted V precipitation, the observed (black diamonds) and
  best-fit kappa distribution (blue }\emph{\DIFdel{x}} %DIFAUXCMD
\DIFdel{symbols) densities and temperatures
  are within these typical ranges; best-fit Maxwellian densities are also within
  the corresponding typically observed range, but the range of best-fit
  Maxwellian temperatures is somewhat low (20--100~eV) compared to the
  corresponding typical range in the plasma sheet.
...
\DIFdelend % ---------------- FIGURE 3
  ...
  \DIFaddbegin \DIFadd{dashed }\DIFaddend line, \ref{ch6:Fig1}b) during this period.

  The top panel of Figure~\ref{ch6:Fig3} displays contours of
  $\chi^2_{\textrm{red}} ( \kappa, R_B )$, where
  \begin{subequations}
    \begin{align} \chi^2_{\mathrm{red}} = \DIFdelbegin %DIFDELCMD < \dfrac{\sum_k \Big ( j_{\parallel,i k} (\Delta \Phi) - j_{\textrm{pred}\parallel,i k}(\Delta \Phi; P_m) \Big )^2 \Big / \sigma_{j_{\parallel,i k}}^2}{\mathrm{dof}}%%%
\DIFdelend \DIFaddbegin \dfrac{\sum_k \Big ( j_{\parallel,i k} (\Delta \Phi) - j_{\textrm{pred}\parallel,i k}(\Delta \Phi; P_m) \Big )^2 \Big / \sigma_{j_{\parallel,i k}}^2}{F}\DIFaddend ; \label{ch6:eqChi2red} \\
    ...
    \end{align}
  \end{subequations}
  \DIFaddbegin \DIFadd{As in (\ref{ch6:eqchi2}), the variable $F$ in (\ref{ch6:eqChi2red}) is the
  degrees of freedom, or the number of $( j_{\parallel,i} , \Delta \Phi )$ pairs
  minus the number of free parameters in $P_m$. }\DIFaddend Parameters with hats in
  ...
  plausible\DIFdelbegin \DIFdel{: one involves a source population that is located at or beyond
  $\sim$4.7~$R_E$ and is extremely nonthermal ($\kappa \lesssim 1.7$) , and the
  other }\DIFdelend \DIFaddbegin \DIFadd{. The first (hereafter ``low-altitude scenario'') }\DIFaddend involves
  acceleration at altitudes near FAST and below $\sim$2.2~$R_E$
  \DIFaddbegin \DIFadd{($R_B \gtrsim$~10) }\DIFaddend with either a thermal or nonthermal source\DIFaddbegin \DIFadd{, and the other
  ...
  nonthermal ($\kappa \lesssim 1.7$)}\DIFaddend .

  \DIFdelbegin \DIFdel{The cause of }\DIFdelend \DIFaddbegin \DIFadd{We interpret }\DIFaddend the large gap in ($\kappa, R_B$) parameter space that separates
  these \DIFdelbegin \DIFdel{scenarios is bifold; each has }\DIFdelend \DIFaddbegin \DIFadd{two scenarios as arising primarily from three effects; two have }\DIFaddend to do
  with the linearity condition \DIFdelbegin \DIFdel{$\Delta \Phi / k_B T \ll R_B $. First, relative }\DIFdelend \DIFaddbegin \DIFadd{$\bar{\phi} \ll R_B $, and the third is a
  consequence of the relationship (\ref{ch6:eqnM}) between $n_m$, $R_{B,F}$ and
  $n_F$.
}

  \begin{enumerate}
  \item \DIFadd{Relative }\DIFaddend to the Maxwellian J-V relation (\ref{ch6:eqKnight}) \DIFdelbegin \DIFdel{, for
    $\kappa \lesssim 2.5$ the }\DIFdelend \DIFaddbegin \DIFadd{for
    far-equilibrium states ($\kappa < \kappa_t \simeq$~2.45) the influence of
    current saturation effects on the }\DIFaddend kappa J-V relation (\ref{ch6:eqDors})\DIFdelbegin \DIFdel{is greatly influenced by saturation effects}\DIFdelend ,
    which arise as the linearity condition is violated\DIFdelbegin \DIFdel{. Second, for }\DIFdelend \DIFaddbegin \DIFadd{, is increasingly strong
  ...
  }\item \DIFadd{Assuming }\DIFaddend equal number density and temperature \DIFaddbegin \DIFadd{and that the linearity
    condition is satisfied}\DIFaddend , the current density predicted by the kappa J-V
    relation (\ref{ch6:eqDors}) \DIFdelbegin \DIFdel{with $\kappa \simeq 1.65$ }\DIFdelend \DIFaddbegin \DIFadd{increases with increasing $\kappa$. For example,
    the current density predicted by the kappa J-V relation }\DIFaddend is more than double
    \DIFdelbegin \DIFdel{the current
  density }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend predicted by the Maxwellian J-V relation (\ref{ch6:eqKnight}) \DIFdelbegin \DIFdel{at
  }\DIFdelend \DIFaddbegin \DIFadd{for
    $\kappa =$~1.65 and }\DIFaddend $\bar{\phi} =$~\DIFdelbegin \DIFdel{10, for example. }\DIFdelend \DIFaddbegin \DIFadd{10.
  }\item \DIFadd{The sigmoid function $Q (\bar{\phi}, \alpha_F)$ in (\ref{ch6:eqBarbosa})
    dictates that the source density $n_m$ monotonically decreases with
    increasing $R_B$.
  }\end{enumerate}
\DIFaddend 

  The \DIFaddbegin \DIFadd{low-altitude scenario, then, arises as follows. For all near-equilibrium
  ...
  \DIFadd{During the period between red bars in Figure~\ref{ch6:Fig1},
  ...
  \DIFadd{The bottom panel in Figure~\ref{ch6:Fig3} shows the observed J-V relation
  (black plus signs, with error bars calculated as described in
  Appendix~\ref{app:A}), and }\DIFaddend best-fit kappa \DIFaddbegin \DIFadd{and Maxwellian }\DIFaddend J-V \DIFaddbegin \DIFadd{relations (blue
  ...
  \DIFadd{The best-fit kappa J-V }\DIFaddend relation (defined as that for which
  ...
  \DIFdelbegin \DIFdel{The bottom panel in Figure~\ref{ch6:Fig3} shows the observed J-V relation
  (black plus signs, with error bars calculated as described in
...
%DIFDELCMD <   %%%
\DIFdelend % SOME LINES ABOUT WHY REDUCED CHI-SQ IS BADDDDD 
  ...
  %DIF <  the linearity condition $\Delta \Phi / k_B T \ll R_B $. First, relative to the
  %DIF >  the linearity condition $\Delta \Phi / T \ll R_B $. First, relative to the
  ...
  \DIFdelbegin \DIFdel{As a result of these effects, a highly nonthermal source population is
...
}\DIFdelend %DIF >  As a result of these effects, a highly nonthermal source population is
  %DIF >  plausible in the top panel of Figure~\ref{ch6:Fig3} in two cases: (1) The
  %DIF >  source population is distant ($h \gtrsim$~4.7~$R_E$) so that according to
  %DIF >  equation (\ref{ch6:eqBarbosa}) the source density is reduced relative to the
  %DIF >  density observed at FAST. The much higher current predicted by the kappa J-V
  %DIF >  relation for extremely nonthermal $\kappa$ values compensates for the reduced
  %DIF >  number density. (2) The source population is nearby ($h <$~2.2~$R_E$) so that
  %DIF >  the much higher number density compensates for current saturation effects.

  \DIFdelbegin \DIFdel{The distinction to be borne in mind is that according to
  ...
  nearby source region. Importantly, the }\DIFdelend %DIF >  The distinction to be borne in mind is that according to
  %DIF >  $\chi^2_{\mathrm{red}}$ values shown in the top panel of \ref{ch6:Fig3} a
  %DIF >  highly nonthermal population is compatible with a high-altitude scenario and
  %DIF >  marginally compatible with a low-altitude scenario, but a source in or near
  %DIF >  thermal equilibrium is only compatible with a low-altitude
  %DIF >  scenario. 
\DIFaddbegin 

  \DIFadd{The }\DIFaddend electron distributions in Figure~\ref{ch6:Fig2} exhibit a
  very pronounced high-energy tail, which argues strongly against a source \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{near
  }\DIFaddend thermal equilibrium and therefore against \DIFdelbegin \DIFdel{the
  scenario involving a nearby ($h <$~2.2~$R_E$) source in thermal equilbrium. }%DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdel{If the assumed preacceleration scenario }\DIFdelend \DIFaddbegin \DIFadd{a low-altitude scenario. Furthermore, if
  preacceleration as }\DIFaddend described in section~\ref{ssSourceAlt} is \DIFdelbegin \DIFdel{correct for }\DIFdelend \DIFaddbegin \DIFadd{a correct
  interpretation of }\DIFaddend the precipitating electrons observed during the marked
  period in Figure~\ref{ch6:Fig1}, with $E_p \simeq 850$~\DIFdelbegin \DIFdel{V }\DIFdelend \DIFaddbegin \DIFadd{eV }\DIFaddend and $T =$~620~eV,
  ...
  consistent with the \DIFdelbegin \DIFdel{scenario involving }\DIFdelend \DIFaddbegin \DIFadd{high-altitude scenario, which involves }\DIFaddend a highly nonthermal
  source at $h \gtrsim$~4.7~$R_E$\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, but is }\DIFaddend marginally inconsistent with the
  \DIFaddbegin \DIFadd{low-altitude }\DIFaddend scenario involving a \DIFdelbegin \DIFdel{thermal source population }\DIFdelend \DIFaddbegin \DIFadd{source population near thermal equilibrium
  }\DIFaddend at $h \lesssim$~2.2~$R_E$.
...
%   Figure~\ref{ch6:Fig1}a
  ...
  Figure~\ref{ch6:Fig5} \DIFaddbegin \DIFadd{is in the same format as Figure~\ref{ch6:Fig2}. The top
  panel }\DIFaddend shows a typical example of the 2-D electron distributions observed
  ...
  Best-fit parameters shown in Figure\DIFdelbegin \DIFdel{suggest }\DIFdelend \DIFaddbegin \DIFadd{~\ref{ch6:Fig4}c indicate }\DIFaddend that
  ...
  format as the top panel of Figure~\ref{ch6:Fig3}. Two scenarios \DIFaddbegin \DIFadd{again }\DIFaddend appear
  most likely: \DIFdelbegin \DIFdel{(1) an extremely }\DIFdelend \DIFaddbegin \DIFadd{a low-altitude scenario involving a thermal or perhaps slightly
  }\DIFaddend nonthermal source at or \DIFdelbegin \DIFdel{above }\DIFdelend \DIFaddbegin \DIFadd{below }\DIFaddend $\sim$\DIFdelbegin \DIFdel{3.7}\DIFdelend \DIFaddbegin \DIFadd{2.6}\DIFaddend ~$R_E$, and \DIFdelbegin \DIFdel{(2) a thermal or perhaps slightly }\DIFdelend \DIFaddbegin \DIFadd{a high-altitude scenario
  involving an extremely }\DIFaddend nonthermal source at or \DIFdelbegin \DIFdel{below
  }\DIFdelend \DIFaddbegin \DIFadd{above }\DIFaddend $\sim$\DIFdelbegin \DIFdel{2.6}\DIFdelend \DIFaddbegin \DIFadd{3.7}\DIFaddend ~$R_E$.
  ...
  that $R_B$ cannot exceed $\sim$22, the overall likelihood of the \DIFdelbegin \DIFdel{first
  scenario involving a }\DIFdelend high-altitude
  \DIFdelbegin \DIFdel{nonthermal source }\DIFdelend \DIFaddbegin \DIFadd{scenario }\DIFaddend is remote. Instead \DIFdelbegin \DIFdel{, }\DIFdelend the balance of the evidence in this case is in
  favor of a \DIFdelbegin \DIFdel{weakly nonthermal }\DIFdelend \DIFaddbegin \DIFadd{low-altitude scenario, which involves a }\DIFaddend source population near
  \DIFaddbegin \DIFadd{thermal equilibrium and near }\DIFaddend or below 2.5~$R_E$. This conclusion about the
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \begin{figure}
    \centering
    \noindent\includegraphics[width=0.85\textwidth]{./ch6/figs/orb_1773__local_analysis_v3_20170614__avgItvl2--trim}

    \vspace{1cm}

    \noindent\includegraphics[width=0.85\textwidth]{./ch6/figs/orb_1773__local_analysis--T_and_N_v3_20170614__avgItvl2--trim}
  ...
  \begin{figure}
    ...
    \noindent\includegraphics[width=0.85\textwidth]{./ch6/figs/orb_1843__local_analysis_v3_20170614--trim}

    \vspace{1cm}

    \noindent\includegraphics[width=0.85\textwidth]{./ch6/figs/orb_1843__local_analysis--T_and_N_v3_20170614--trim}
  ...
  altitudes $h \gtrsim$~3.8~$R_E$. Neither scenario matches \DIFdelbegin \DIFdel{any of the three
  }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend scenarios in the
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  local \DIFdelbegin \DIFdel{depression }\DIFdelend \DIFaddbegin \DIFadd{valley }\DIFaddend in $\chi^2_{\mathrm{red}} (\kappa, R_B)$ for
  ...
  are greater than those between which the \DIFdelbegin \DIFdel{depression }\DIFdelend \DIFaddbegin \DIFadd{valley presently }\DIFaddend appears, and
  therefore that \DIFdelbegin \DIFdel{our conclusion that }\DIFdelend a thermal source \DIFdelbegin \DIFdel{is marginally inconsistent }\DIFdelend \DIFaddbegin \DIFadd{may be marginally consistent (rather than
  marginally inconsistent, as we have suggested) }\DIFaddend with the thermal mirror ratio
  $R_{B,T} = 9$\DIFdelbegin \DIFdel{is unwarranted}\DIFdelend .
...
%     hard to keep up with the text you on this. }
  ...
  We are grateful to Craig Markwardt for making publicly available \DIFdelbegin \DIFdel{an
  }\DIFdelend \DIFaddbegin \DIFadd{MPFIT, the
  }\DIFaddend Interactive Data Language version of the MINPACK-1 fitting routines, which we
