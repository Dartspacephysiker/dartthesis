...
that inside the symbol $\grad$, the differential operator
$\partial / \partial y = 0$). I won't outrightly say that the $y$ coordinate
doesn't exist, since, for example, the curl operator $\grad \times$ is a
...
\begin{equation} \label{ch1:eqCont1} \dfrac{\partial}{\partial t} \big (
  \cancel{q_i n_i + q_e n_e} \big ) + \grad \cdot \big ( q_{e} n_{e}
  \mathbf{v}_{e} + q_i n_i \mathbf{v}_i \big ) = 0.
\end{equation}
If we now recall the quasineutrality condition (\ref{ch1:eqQuasi}), you can see
that the divergence term (that's the symbol $\grad \cdot$ and all the stuff in
parentheses that follows after) is $\grad \cdot \mathbf{j}$, where $\mathbf{j}
\equiv q n ( \mathbf{v}_i - \mathbf{v}_e )$.

For to-be-disclosed reasons, I take the time derivative of (\ref{ch1:eqCont1})
so that
...
Why don't we take a look at the $y$ component of the momentum equations
(\ref{ch1:eqMom})? This suggestion might seem strange since I've explicitly assumed that
$\partial / \partial y = 0$, but never mind: writing out the $y$ components,
we find that
\begin{equation} 
  \begin{aligned}
    \label{ch1:eqMomy} \dfrac{\partial v_{\textrm{e}y}}{\partial t} &=
    \dfrac{q_e}{m_e}v_{\textrm{e}x} B_o \\
    \dfrac{\partial v_{\textrm{e}y}}{\partial t} &=
    \dfrac{q_e}{m_i}v_{\textrm{i}x} B_o \\
    \implies v_{\textrm{e}x} = -
    \dfrac{m_e}{m_i} v_{\textrm{i}x}.
  \end{aligned}
\end{equation}
Ah, so! It turns out that in the $x$ direction, electron motion is opposite that
of the ions at a speed that is less than that of ions by a factor of
$m_e / m_i$. (In plain English: electrons move much, much more slowly than ions
in the $x$ direction.) If you were skeptical about looking at the $y$ component
(''There's no variation in the $y$ direction, quit wasting time!'') you'd better
take a moment to reexamine (\ref{ch1:eqMomy}) to see what just happened.

The main question is, why the shortage of a species-specifying subscript
(alliterate!) on $v_y$? Doubtless you remember all those things I was saying
about making life easy via simplifying assumptions; $v_y$ is Exhibit A, a
ramification of my decision to set $\partial / \partial y = 0$---or rather, the
assumption $\partial / \partial y = 0$ obviously means that
$\partial v_{\textrm{e}y} / \partial y = \partial v_{\textrm{i}y} / \partial y =
0$,
and that there is no need for specifying a species because derivatives with
respect to $y$ are all zero anyway. 

``Should we care?'' Yes! We do care! The beauty is that we can solve both of the
expressions (\ref{ch1:eqMomx}) for $v_y$


Why don't we take a look at the $x$ component of the momentum equations (\ref{ch1:eqMom})? They are
\begin{equation}
...
[Show parallel electron acceleration from
$E_z$ ]
