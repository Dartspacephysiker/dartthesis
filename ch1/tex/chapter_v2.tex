...
So much for spatial variation. How about variation in time? I shouldn't even
talk about temporal variation without first introducing you to a
...
as a consequence of the magnetic force
\begin{equation} \label{ch1:eqMagForce} 
\mathbf{F}_m = q_\alpha \mathbf{v}_\alpha \times \mathbf{B}.
\end{equation} 
...
cyclotron frequency $\omega_{c \alpha} = \vert q_\alpha B /m_\alpha
\vert$. Also, see how the direction of $\mathbf{F}_m$ depends on the sign of
$q_\alpha$? It means that the force due to the magnetic field $\mathbf{B}$ on
electrons, which are negatively charged, is in a direction opposite the force
exerted on ions, which are positively charged; electrons and ions thus gyrate
about the magnetic field line in opposite senses.

Back to our question about variation in time. You can see that
$\omega_{ci} / \omega_{ce} = m_e/m_i$, or that the ion-cyclotron frequency
$\omega_{ci}$ is much less than the electron-cyclotron frequency $\omega_{ce}$,
and thus we have at least three timescales that we \emph{could} be interested in, \par
\begin{enumerate}
\item \makebox[4.5cm]{\qquad $ \partial / \partial t \gg \omega_{ce} $} (high frequencies),
\item \makebox[4.5cm]{$ \omega_{ci} < \partial / \partial t < \omega_{ce} $} (intermediate frequencies),
\item \makebox[4.5cm]{\qquad $ \partial / \partial t \ll \omega_{ci} $} (low frequencies).
\end{enumerate}
We'll assume that the electric and magnetic field as well as the particles in
our system vary at low frequencies (option \#3) so that
$\partial^2 / \partial t^2 \ll \omega_{ci}^2$. 

We're also going to assume the existence of an electric field $\mathbf{E}$ that
only varies in the $x-z$ plane: $\mathbf{E} = \mathbf{E} (x, z, t)$.
...
$\grad \cdot \mathbf{j}$ :
\begin{equation*} 
  \dfrac{\partial}{\partial t} \Big ( \cancel{n_i - n_e} \Big )
  + \grad \cdot \Big (n_i \mathbf{v}_i - n_{e} \mathbf{v}_{e} \Big ) = \grad
  \cdot \mathbf{j} = 0.
\end{equation*}
...
\emph{as\'{i}}:
\begin{equation} 
  \label{ch1:eqCont2} \dfrac{\partial^2 j_x}{\partial x \partial t} + \dfrac{\partial^2 j_z}{\partial z \partial t} = 0.
\end{equation}
...
(\ref{ch1:eqMom})? It is
% \begin{equation}
%   \begin{alignedat}{3} \label{ch1:eqMomx} \dfrac{\partial v_{\textrm{ex}}}{\partial t} &= - &\dfrac{q}{m_e} E_x + \omega_{ce} v_y; \\
%     \dfrac{\partial v_{\textrm{ix}}}{\partial t} &= &\dfrac{q}{m_i} E_x + \omega_{ci} v_y.
%   \end{alignedat}
% \end{equation}
\begin{equation} \label{ch1:eqMomx} \dfrac{\partial v_{x,\alpha}}{\partial t} =
  \dfrac{q_\alpha}{m_\alpha} \Big ( E_x + v_{y,\alpha} B_o - \cancel{v_{z,\alpha} B_y}
  \Big ) = \dfrac{q_\alpha}{m_\alpha} E_x \pm \omega_{c \alpha} v_{y,\alpha},
\end{equation}
where I've junked the $v_{z,\alpha} B_y$ term because it is \emph{nonlinear}
...
$\frac{\partial v_{y,\alpha}}{\partial t}$ will appear on the right-hand side?
I can then ladle in (\ref{ch1:eqMomy}) for $\frac{\partial
  v_{y,\alpha}}{\partial t}$,
% \begin{equation}
...
%   % \begin{aligned} \label{ch1:eqMomxdt} 
% \begin{alignedat}{5} \label{ch1:eqMomxdt} 
%     \dfrac{\partial^2 v_{\textrm{ex}}}{\partial t^2} &= - &\dfrac{q}{m_e}\dfrac{\partial E_x}{\partial t} - \omega_{ce} \dfrac{\partial v_{\textrm{e}y}}{\partial t} &= -&\dfrac{q}{m_e}\dfrac{\partial E_x}{\partial t} + \omega_{ce}^2 v_{\textrm{e}x}; \\
%     \dfrac{\partial^2 v_{\textrm{ix}}}{\partial t^2} &= &\dfrac{q}{m_i}\dfrac{\partial E_x}{\partial t} + \omega_{ci} \dfrac{\partial v_{\textrm{i}y}}{\partial t} &= &\dfrac{q}{m_i}\dfrac{\partial E_x}{\partial t} + \omega_{ci}^2 v_{\textrm{i}x}.
% \end{alignedat}
% %   \end{aligned}
% \end{equation}
\begin{equation*} \label{ch1:eqMomxdt} \dfrac{\partial^2 v_{x,\alpha}}{\partial
    t^2} = \dfrac{q_\alpha}{m_\alpha}\dfrac{\partial E_x}{\partial t} \pm
  \omega_{c \alpha} \dfrac{\partial v_{y,\alpha}}{\partial t} =
...
\begin{equation*} \Big ( \dfrac{\partial^2}{\partial t^2} + \omega_{c \alpha}^2
  \Big ) v_{x,\alpha} = \dfrac{q_\alpha}{m_\alpha}\dfrac{\partial E_x}{\partial
    t},
\end{equation*}
...
\begin{equation} \label{ch1:eqvxEx}
  v_{x,\alpha} = \pm \dfrac{1}{\omega_{c \alpha} B_o}\dfrac{\partial
    E_x}{\partial t}.
\end{equation}
...
\begin{equation} \label{ch1:eqvzEz} \dfrac{\partial v_{z,\alpha}}{\partial t} =
  \dfrac{q_\alpha}{m_\alpha} \Big ( E_z + \cancel{v_{x,\alpha} B_y} -
  \cancel{v_{y,\alpha} B_x} \Big ) = \pm \dfrac{\omega_{c \alpha}}{B_o} E_z.
\end{equation}
Baller, right? For my next trick, I'll plug (\ref{ch1:eqvxEx}) and
(\ref{ch1:eqvzEz}) into the appropriate components of $\mathbf{j}$,
\begin{equation*} 
  \begin{alignedat}{4}
    j_x                              &= q &\Big ( v_{x,i} - &v_{x,e} \Big ) &= \dfrac{1}{B_o^2} &\Big( m_i + m_e \Big) \dfrac{\partial E_x}{\partial t}; \\
    \dfrac{\partial j_z}{\partial t} &= q \dfrac{\partial}{\partial t} &\Big ( v_{z,i} - &v_{z,e} \Big ) &= q^2 &\Big( \dfrac{1}{m_i} + \dfrac{1}{m_e} \Big) E_z. \\
  \end{alignedat}
\end{equation*}
...
pull at the expressions for $j_x$ and $j_z$ that I just produced to try to make
the combination $m_e/m_i$ appear. When it does, I blast the whole term to which
it is joined into next week (i.e., set it to zero)! Here, take my pistol and
give it a shot; if you do, you should find
\begin{equation} \label{ch1:eqDaddy}
  \begin{alignedat}{4}
    j_x                              &= \dfrac{m_i}{B_o^2} \dfrac{\partial E_x}{\partial t}; \\
    \dfrac{\partial j_z}{\partial t} &= \dfrac{q^2}{m_e} E_z. \\
  \end{alignedat}
\end{equation}
...
But (in the words of Robert Frost) we're out for stars! First we enter a
veritable thicket of math, in which you are almost definitely going to lose your
sense of direction if you've never before traversed such terrain. If I plug the
expressions (\ref{ch1:eqDaddy}) for $j_x$ and $j_z$ into (\ref{ch1:eqCont2}), I
obtain the seemingly meaningless result
\begin{equation*}
  % \begin{alignedat}{4}
    \dfrac{\partial^2}{\partial t \partial x} j_x + \dfrac{\partial}{\partial z}
    \Big( \dfrac{\partial j_z}{\partial t} \Big) = \dfrac{\partial^2}{\partial
      t \partial x} \Big(\dfrac{m_i}{B_o^2}\dfrac{\partial E_x}{\partial t} \Big) +
    \dfrac{\partial}{\partial z} \Big( \dfrac{q^2}{m_e} E_z \Big) = 0.
  % \end{alignedat}
\end{equation*}
If I multiply the whole thing by $\mu_o c^2 n$ I get 
\begin{equation*}
...
$\omega_{pe}$, that leathery looking chap standing by $E_z$?

Take heart, tenderfoot. we're near the top.

OK, let's take it to the top! Plugging 
