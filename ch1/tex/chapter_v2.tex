...
% Check footskip
% My footskip is \the\footskip. (Prints as "My footskip is 30.0pt.")
...
\begin{figure}
  ...
  \noindent\includegraphics[width=0.9\textwidth]{./ch1/figs/EugenKempf.jpg}
...
$v_A =$~(nonsense).\footnote{Instead of just writing the situation off as
  ...
  % $\omega / k = c \sqrt{B^2/\mu_0 / (B^2/\mu_0 + \epsilon + P)}$. (Gonna have to
...
% did $a$: a background value, $b_0 =$~2000, and a perturbation value, $\tilde b
% \equiv b - b_0 =$~50.
...
Question: What is the relative contribution of each of the three terms $a_0^2$,
$2 a_0 \tilde a$, and $\tilde a^2$ to the full value, $a^2$?
\begin{itemize}
\item \makebox[6.5cm]{$ a_0^2 =$~1} (0th order in $\tilde a$),
\item \makebox[6.5cm]{$ 2 a_0 \tilde a = 0.1 $ } (first order in $\tilde a$),
...
\begin{itemize}
\item \makebox[6.5cm]{$ n = n_0 + \tilde n $} (number density),
\item \makebox[6.5cm]{$ \mathbf{v} = \cancel{\mathbf{v}_0} + \mathbf{\tilde v} $ } (velocity),
\item \makebox[6.5cm]{$ \mathbf{B} = \mathbf{B}_0 + \mathbf{b} = B_0 \hat{z} + \langle b_x, b_y, b_z \rangle $} (magnetic field),
...
number flux
\begin{equation}
  % n \mathbf{v} = \Big ( n_0 + \tilde n \Big ) \Big ( \mathbf{v}_0 + \mathbf{\tilde v} \Big ) = n_0 \mathbf{v}_0 + n_0 \mathbf{\tilde v} + \tilde n \mathbf{v}_0 + \tilde n \mathbf{\tilde v}.
...
\begin{equation}
  % n \mathbf{v} = n_0 \mathbf{v}_0 + n_0 \mathbf{\tilde v} + \tilde n \mathbf{v}_0.
...
\emph{are} laws. As far as I know, they are genuinely immutable.% \footnote{Some
  ...
  % $\mu_0 \epsilon_0 \frac{\partial \mathbf{E}}{\partial t}$. We're assuming our
...
First of all, I will assume a singly charged ion species so that $q_i = q$ and
...
What we're ultimately aiming for is an equation that is only in terms
