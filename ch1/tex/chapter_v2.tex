...
Now for the fun part. We get to make a bunch of assumptions, whittling away at
(\ref{ch1:eqCont}--\ref{ch1:eqFar}) as we go. First of all, let's set up a
Cartesian coordinate system\footnote{We could use a dipolar coordinate system,
  as do \citet{Lysak2013}, but I'd rather keep it simple for the sake of both of
  us.} such that the $x$ coordinate is perpendicular to the local geomagnetic
field, the $y$ coordinate is negligible (i.e., nothing varies in the $y$
direction, which means that inside the symbol $\grad$, the differential operator
$\partial / \partial y = 0$), and the $z$ coordinate is along the local
geomagnetic field. We won't outrightly say that the $y$ coordinate doesn't
exist, since that would render meaningless the three-dimensional definitions of,
for example, the cross product and curl operator.
...
For to-be-disclosed reasons, let's take the time derivative of
(\ref{ch1:eqCont1}) so that
...
Turning now to the momentum equations (\ref{ch1:eqMom})
