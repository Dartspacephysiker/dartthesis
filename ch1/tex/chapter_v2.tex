...
\begin{itemize}
\item \makebox[4.5cm]{$ n = n_o + \tilde n $} (number density),
\item \makebox[4.5cm]{$ \mathbf{v} = \mathbf{v}_o + \mathbf{\tilde v} = \mathbf{ \tilde v} $ } (velocity),
\item \makebox[4.5cm]{$ \mathbf{B} = \mathbf{B}_o + \mathbf{b} = B_o \hat{z} + \langle b_x, b_y, b_z \rangle $} (magnetic field).
\end{itemize}
...
let $\tilde n_{\alpha}(\mathbf{x},t)$ change how it will.\footnote{In math speak
  I am saying that from eternity to eternity,
...
\end{equation*}
See how nicely that'll slip right into (\ref{ch1:eqMeaningless2})? Go ahead and
slap it in, multiplying both sides by $\frac{\omega_{pe}^2}{v_A^2}$ as you go, to find
\begin{equation*}
